      ...
      <div className="min-h-[calc(100vh-88px)] bg-[#fafbfc] p-[30px]">
        <div>
          ...
          <p className="mt-[5px] text-[12px] text-[#626262]">
            Kelola stok, batch, expired date, stock movement, opname, dan
            supplier.
          </p>
        </div>
  ...
  return (
    ...
    >
      ...
      <Table>
        ...
        <tbody>
          {loading ? (
            <Empty colSpan={6} text="Memuat stok..." />
          ) : rows.length === 0 ? (
            <Empty colSpan={6} text="Data stok belum tersedia." />
          ) : (
            rows.map((row) => (
              <tr key={row.id} className="border-b border-[#f1f1f1]">
                ...
                <Td>
                  {formatStock(row.usable_stock)} {row.unit ?? ""}
                </Td>
...
function BatchTab() {
  ...
  const receiveStock = async (batch) => {
    const input = window.prompt(`Tambah stock batch ${batch.batch_number}:`);
        ...
        <Table>
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((batch) => (
                <tr key={batch.id} className="border-b border-[#f1f1f1]">
...
function OpnameTab() {
  ...
  const selectableBatches = useMemo(
    () =>
      batches.filter(
        (batch) =>
          !items.some((item) => Number(item.batch_id) === Number(batch.id)),
      ),
    [batches, items],
  );
  ...
  const removeItem = (batchId) => {
    setItems((previous) =>
      previous.filter((item) => Number(item.batch_id) !== Number(batchId)),
    );
  };
    ...
    >
        ...
        >
          ...
          {selectableBatches.map((batch) => (
            <option key={batch.id} value={batch.id}>
              {batch?.medicine?.name ?? "-"} — {batch.batch_number} — System{" "}
              {batch.stock}
            </option>
          ))}
        </select>
      ...
      {items.length > 0 && (
        <div className="mt-[16px] space-y-[8px]">
          {items.map((item) => {
            const difference =
              Number(item.physical_stock) - Number(item.batch?.stock ?? 0);
              ...
              >
                <div>
                  ...
                  <p className="mt-[2px] text-[11px] text-[#626262]">
                    • System {formatStock(item.batch.stock)}• Fisik{" "}
                    {formatStock(item.physical_stock)}• Selisih{" "}
                    {formatStock(difference)}
                  </p>
                </div>
    ...
    <div className="space-y-[18px]">
      ...
      <Card title="Daftar Supplier">
        <Table>
          ...
          <tbody>
            {rows.length === 0 ? (
              <Empty colSpan={6} text="Supplier belum tersedia." />
            ) : (
              rows.map((supplier) => (
                <tr key={supplier.id} className="border-b border-[#f1f1f1]">
                  <Td>{supplier.code}</Td>
  ...
  return (
    <section className="overflow-hidden rounded-[14px] border border-[#ececec] bg-white">
      <div className="border-b border-[#eeeeee] px-[20px] py-[16px]">
        ...
        {subtitle && (
          <p className="mt-[3px] text-[12px] text-[#626262]">{subtitle}</p>
        )}
      </div>
